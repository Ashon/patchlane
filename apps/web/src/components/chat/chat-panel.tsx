...
export const ChatPanel = ({ endpoint }: ChatPanelProps) => {
  ...
  return (
    <ChatConversation
      ...
      error={error}
      header={
        <header className="flex flex-col gap-2 border-b bg-card px-3 py-2 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Button disabled={!messages.length && !error} onClick={clearChat} size="sm" type="button" variant="outline">
              Clear
            </Button>
          </div>
        </header>
      ...
      inputActions={
        isStreaming ? (
          ...
          <PromptInputAction tooltip="Send message">
            <Button disabled={!canChat || !input.trim()} onClick={() => void sendMessage()} size="icon" type="button">
              <Send />
            </Button>
          </PromptInputAction>
        )
      ...
      inputPlaceholder={canChat ? "Ask the selected model..." : "Select an enabled endpoint first"}
      inputValue={input}
      messages={messages}
