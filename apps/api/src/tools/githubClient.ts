...
export const testGitHubToken = async (token: string): Promise<GitHubToolTestResult> => {
  ...
  try {
    ...
    const rateLimitRemaining = parseHeaderInt(response.headers.get("x-ratelimit-remaining"));
