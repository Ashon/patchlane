import type { AgentProject, Issue, LlmEndpoint, SandboxFileEntry, SandboxWorkspace } from "@agent-fleet/shared";
...
export const analyzeIssueWithPlanningAgent = async (input: IssueAnalysisAgentInput): Promise<IssuePlanningAgentResult> => {
...
export const analyzeIssueRequirements = async (input: IssueAnalysisAgentInput) => {
...
export const planIssueWork = async (input: IssueAnalysisAgentInput, requirementAnalysis: string) => {
  return runPlanningCompletion({
    input,
    maxTokens: 1_400,
...
export const combineIssuePlanningAnalysis = (requirementAnalysis: string, workPlan: string): IssuePlanningAgentResult => {
  return {
...
const buildWorkPlanPrompt = (input: IssueAnalysisAgentInput, requirementAnalysis: string) => {
  return [
...
const formatFileEntries = (entries: SandboxFileEntry[]) => {
  ...
  return entries
    ...
    .map((entry) => `- ${entry.type === "directory" ? "dir" : "file"} ${entry.path}${entry.size ? ` (${entry.size} bytes)` : ""}`)
    .join("\n");
};
