...
export const reconcileIssuePlanningState = async ({
  ...
  const requirementRun = issue.requirementRunId ? await runStore.find(issue.requirementRunId) : undefined;

  if (requirementRun) {
    runs.push(requirementRun);
  }

  const completedRequirementAnalysis = requirementRun?.status === "completed" ? getAgentRunResultText(requirementRun) : undefined;
  const requirementAnalysis = completedRequirementAnalysis ?? buildRecoveredRequirementContext(issue, requirementRun);
  ...
  const promotedIssue = await issueStore.analyzeIssue(issue.id, {
    ...
    endpointId: issue.endpointId ?? planningRun.endpointId ?? requirementRun?.endpointId,
...
export const reconcileIssueAfterAgentRun = async (
  ...
  if (!run.issueId || (run.kind !== "requirements" && run.kind !== "planning")) {
    return undefined;
  }
...
export const getAgentRunResultText = (run: AgentRun) => {
  ...
  return candidates.map(cleanResultText).find((candidate) => candidate && isUsableResultText(candidate));
};

const buildRecoveredRequirementContext = (issue: Issue, requirementRun?: AgentRun) => {
  return [
    "### Requirement Summary",
...
const isUsableResultText = (value: string) => {
  return !value.startsWith("Tool iteration limit reached") && !value.startsWith("Planning failed.");
};
