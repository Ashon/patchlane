...
import type { SandboxFileContent, SandboxFileEntry, SandboxWorkspace } from "@agent-fleet/shared";
import { badRequest } from "../http/errors";
import { ensureWorkspacePath } from "./sandboxExecutor";

const maxReadableFileBytes = 1_000_000;
const ignoredDirectoryNames = new Set([".git", "node_modules", "dist", "build", ".next", "coverage"]);

export const listWorkspaceFiles = async (workspace: SandboxWorkspace, requestedPath = "."): Promise<SandboxFileEntry[]> => {
...
export const readWorkspaceFile = async (workspace: SandboxWorkspace, requestedPath: string): Promise<SandboxFileContent> => {
  const absolutePath = ensureWorkspacePath(workspace, requestedPath);
