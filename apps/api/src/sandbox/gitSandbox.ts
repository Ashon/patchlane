...
export const ensureRepositoryCache = async ({
  ...
  if (!(await pathExists(path.join(target.path, ".git")))) {
    ...
    if (!remoteResult.ok) {
      throw new Error(getFailureMessage("git remote set-url", remoteResult.stderr || remoteResult.stdout));
    }
  }
  ...
  if (!fetchResult.ok) {
    throw new Error(getFailureMessage("git fetch", fetchResult.stderr || fetchResult.stdout));
  }

  const resolvedRef = await resolveBaseRef({ cwd: target.path, env, ref, settings });
  const checkoutResult = await runProcess({
    command: "git",
...
export const createWorktreeFromCache = async ({
  ...
  const env = buildSandboxEnv(settings.envAllowlist, getGitAuthEnv(cache.repositoryUrl, githubToken));
  const fetchResult = await runProcess({
    command: "git",
    args: ["fetch", "--prune", "origin"],
    cwd: cache.path,
  ...
  const resolvedRef = await resolveBaseRef({ cwd: cache.path, env, ref: baseRef ?? cache.ref, settings });
  await mkdir(path.dirname(target.path), { recursive: true });
  ...
  if (!worktreeResult.ok) {
    throw new Error(getFailureMessage("git worktree add", worktreeResult.stderr || worktreeResult.stdout));
  }

  return resolvedRef;
};

export const removeWorktreeFromCache = async ({ cache, githubToken, settings, target }: RemoveWorktreeInput) => {
  if (!cache.repositoryUrl) {
    return;
  }
  ...
  if (!removeResult.ok && (await pathExists(target.path))) {
    throw new Error(getFailureMessage("git worktree remove", removeResult.stderr || removeResult.stdout));
  }
