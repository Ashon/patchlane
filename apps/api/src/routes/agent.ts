...
export const createAgentRouter = ({
  ...
  router.get(
    ...
    asyncHandler(async (request, response) => {
      response.json({ run: await runStore.get(getRouteParam(request.params.id, "id")) });
    })
  );
  ...
  router.post(
    ...
    asyncHandler(async (request, response) => {
      ...
      const run = await runStore.appendMessage(getRouteParam(request.params.id, "id"), {
        role: "user",
  ...
  router.post(
    ...
    asyncHandler(async (request, response) => {
      ...
      const run = await runtime.continue(getRouteParam(request.params.id, "id"), input.endpointId, input.model);
  ...
  router.post("/runs/:id/continue/stream", async (request, response, next) => {
    ...
    try {
      ...
      await runtime.continueStream(id, input.endpointId, input.model, (event) => {
