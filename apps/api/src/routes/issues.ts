...
export const createIssuesRouter = ({
  ...
  router.post(
    ...
    asyncHandler(async (request, response) => {
      ...
      const id = getRouteParam(request.params.id, 'id')
      const { issue: currentIssue, runs: reconciledRuns } =
        await reconcileIssuePlanningState({
          issueId: id,
          issueStore,
          runStore,
        })
      ...
      if (!isIssueAnalysisReady(currentIssue)) {
        ...
        response.status(202).json({
          ...
          runs: [...reconciledRuns, ...analysisResult.runs],
        })
        return
      }
      ...
      const endpointId =
        input.endpointId ?? readyIssue.endpointId ?? project.defaultEndpointId
  ...
  async function analyzeIssueWithAgent(
    id: string,
    ...
    if (isIssueAnalysisReady(reconciled.issue)) {
      return {
        ...
        runs: reconciled.runs.length
          ? reconciled.runs
          : await getPlanningRuns(reconciled.issue),
      }
    }

    const context = await issueStore.getIssueAnalysisContext(
      id,
      requestedEndpointId,
    )
    const endpoint = context.endpointId
      ? await endpointStore.get(context.endpointId)
      : await endpointStore.getDefault()
    const workspace = context.workspaceId
      ? await workspaceStore.get(context.workspaceId)
      : undefined

    if (!workspace) {
      throw badRequest(
        'Connect a repository or sandbox workspace before creating analysis agent tasks',
      )
    }
    ...
    void completeIssuePlanning({
      ...
      console.error(
        `Issue planning background task crashed for ${context.issue.id}: ${getErrorMessage(error)}`,
      )
    })
    ...
    try {
      ...
      const fileEntries =
        workspace.status === 'ready'
          ? await listWorkspaceFiles(workspace).catch(() => [])
          : []
      const input = {
        branchName,
        endpoint,
      ...
      const requirementWithResult = await runStore.appendMessage(
        requirementRun.id,
        {
          role: 'assistant',
      ...
      const analysis = combineIssuePlanningAnalysis(
        requirementAnalysis,
        workPlan,
      )
  ...
  async function failAgentTask(runId: string, message: string) {
    try {
      ...
      console.error(
        `Could not mark agent task ${runId} as failed: ${getErrorMessage(error)}`,
      )
      return undefined
    }
  }

  async function getActivePlanningRuns(
    ...
    return runs.some((run) => run.status === 'running' || run.status === 'idle')
      ? runs
      : undefined
  }

  async function getPlanningRuns(
    issue: Awaited<ReturnType<IssueStore['getIssue']>>,
  ) {
    const runIds = [issue.requirementRunId, issue.planningRunId].filter(
