...
import { createSandboxWorkspaceSchema, sandboxExecRequestSchema, type SandboxSettings } from "@agent-fleet/shared";
...
export const createSandboxRouter = ({ settings, workspaceStore, toolSettingsStore }: SandboxRouterOptions) => {
  const router = Router();
  ...
  router.post(
    ...
    asyncHandler(async (request, response) => {
      ...
      response.status(201).json({ workspace: await workspaceStore.get(workspace.id) });
    })
  );
  ...
  router.post(
    "/workspaces/:id/exec",
    asyncHandler(async (request, response) => {
      const workspace = await workspaceStore.get(getRouteParam(request.params.id, "id"));
