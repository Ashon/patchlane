...
export class LlmEndpointStore {
  ...
  async getDefault() {
    const row = this.database.sqlite
      .prepare("SELECT * FROM llm_endpoints WHERE enabled = 1 ORDER BY name ASC LIMIT 1")
  ...
  async remove(id: string) {
    const result = this.database.sqlite.prepare("DELETE FROM llm_endpoints WHERE id = ?").run(id);
  ...
  private ensureSeeded() {
    const countRow = this.database.sqlite.prepare("SELECT COUNT(*) AS count FROM llm_endpoints").get() as { count: number };

    if (countRow.count > 0) {
      return;
    }

    const legacyEndpoints = readLegacyJson(this.legacyFilePath, llmEndpointListSchema);
